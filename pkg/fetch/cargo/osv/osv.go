...
const dataURL = "https://osv-vulnerabilities.storage.googleapis.com/crates.io/all.zip"
...
func Fetch(opts ...Option) error {
	...
	for _, zf := range r.File {
		...
		if !ok {
			return errors.Wrapf(err, "unexpected ID format. expected: %q, actual: %q", "(RUSTSEC|GHSA|MAL)-.+", a.ID)
		}
		...
		p, err := utilfilepath.Join(options.dir, lhs, t.Format("2006"), fmt.Sprintf("%s.json", a.ID))
		if err != nil {
