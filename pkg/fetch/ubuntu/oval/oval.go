...
func Fetch(opts ...Option) error {
	...
	for name, href := range ovals {
		...
		dir := filepath.Join(options.dir, release, service)
		...
		for _, variable := range root.Variables.ConstantVariable {
			if err := util.Write(filepath.Join(dir, "variables", "constant_variable", fmt.Sprintf("%s.json", variable.ID)), variable); err != nil {
				return errors.Wrapf(err, "write %s", filepath.Join(dir, "variables", "constant_variable", fmt.Sprintf("%s.json", variable.ID)))
			}
			bar.Increment()
		}
		bar.Finish()
