...
func Fetch(opts ...Option) error {
	options := &options{
		...
		dir:     filepath.Join(util.CacheDir(), "fetch", "ubuntu", "tracker"),
		retry:   3,
	}
		...
		default:
			...
			case "Priority":
				...
				if ok {
					...
					base.Releases[rhs] = Release{Priority: p}
					a.Packages[lhs] = base
				} else {
					if a.Packages == nil {
						a.Packages = make(map[string]Package)
			...
			case "Tags", "-- Tags":
				...
				if ok {
					...
					base.Releases[rhs] = Release{
						Tags: strings.Fields(strings.TrimSpace(tags)),
					}
					a.Packages[lhs] = base
				} else {
					if a.Packages == nil {
