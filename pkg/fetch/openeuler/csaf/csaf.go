	...
	if err := client.PipelineGet(us, o.concurrency, o.wait, func(resp *http.Response) error {
		...
		case http.StatusOK:
			...
			case "cve":
				...
				if err != nil {
					fmt.Println(csaf.Document.Tracking.ID, err)
					return errors.Wrapf(err, "unexpected ID format. expected: %q, actual: %q", "CVE-yyyy-\\d+.json", csaf.Document.Tracking.ID)
				}
			...
			case "advisories":
				splitted, err := util.Split(csaf.Document.Tracking.ID, "-", "-", "-")
				if err != nil {
					return errors.Wrapf(err, "unexpected ID format. expected: %q, actual: %q", "openEuler-SA-yyyy-\\d+.json", csaf.Document.Tracking.ID)
				}
