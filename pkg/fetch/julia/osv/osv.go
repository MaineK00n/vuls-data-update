...
const dataURL = "https://osv-vulnerabilities.storage.googleapis.com/Julia/all.zip"
...
func Fetch(opts ...Option) error {
	...
	for _, zf := range r.File {
		...
		if err != nil {
			return errors.Wrapf(err, "unexpected ID format. expected: %q, actual: %q", "JLSEC-yyyy-\\d+", a.ID)
		}
		if _, err := time.Parse("2006", splitted[1]); err != nil {
			return errors.Wrapf(err, "unexpected ID format. expected: %q, actual: %q", "JLSEC-yyyy-\\d+", a.ID)
		}

		p, err := utilfilepath.Join(options.dir, splitted[1], fmt.Sprintf("%s.json", a.ID))
		if err != nil {
