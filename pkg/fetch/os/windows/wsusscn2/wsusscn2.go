...
func Fetch(opts ...Option) error {
	options := &options{
		dataURL:     dataURL,
		dir:         filepath.Join(util.SourceDir(), "windows", "wsusscn2"),
		retry:       3,
		concurrency: 2,
	}
	...
	for _, u := range pkg.Updates.Update {
		...
		if err := util.Write(filepath.Join(options.dir, "u", fmt.Sprintf("%s.json.gz", u.UpdateID)), u); err != nil {
			return errors.Wrapf(err, "write %s", filepath.Join(options.dir, "u", fmt.Sprintf("%s.json.gz", u.UpdateID)))
		}
		...
		if err := util.Write(filepath.Join(options.dir, "x", fmt.Sprintf("%s.json.gz", u.RevisionID)), x); err != nil {
			return errors.Wrapf(err, "write %s", filepath.Join(options.dir, "x", fmt.Sprintf("%s.json.gz", u.RevisionID)))
		}

		if err := util.Write(filepath.Join(options.dir, "l", fmt.Sprintf("%s.json.gz", u.RevisionID)), l); err != nil {
			return errors.Wrapf(err, "write %s", filepath.Join(options.dir, "l", fmt.Sprintf("%s.json.gz", u.RevisionID)))
		}
	...
	if err := os.RemoveAll(filepath.Dir(rootDir)); err != nil {
		return errors.Wrapf(err, "failed to remove %s", filepath.Dir(rootDir))
	}
