...
func Fetch(opts ...Option) error {
	...
	for _, v := range vuxml.Vuln {
		if err := util.Write(filepath.Join(options.dir, fmt.Sprintf("%s.json.gz", v.Vid)), v); err != nil {
			return errors.Wrapf(err, "write %s", filepath.Join(options.dir, fmt.Sprintf("%s.json.gz", v.Vid)))
		}
