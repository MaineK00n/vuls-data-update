...
func Fetch(opts ...Option) error {
	...
	for name, url := range options.urls {
		...
		verAdvs, err := options.fetchOVAL(url)
		if err != nil {
			return errors.Wrapf(err, "fetch %s", name)
		}

		for v, advs := range verAdvs {
			log.Printf("[INFO] Fetched %s %s", osname, v)
			dir := filepath.Join(options.dir, osname, v)
...
func (opts options) fetchOVAL(url string) (map[string][]Advisory, error) {
...
func parseDefinitions(xmlname string, ovalDefs []definition, tests map[string]Package) map[string][]Advisory {
	...
	for _, d := range ovalDefs {
		...
		case "vulnerability":
			...
			for ver, pkgs := range collectPkgs(xmlname, d.Criteria, tests) {
				for _, cve := range cves {
					advs[ver] = append(advs[ver], Advisory{
						ID:           cve.CveID,
						DefinitionID: d.ID,
						Title:        d.Title,
...
func collectPkgs(xmlname string, cri criteria, tests map[string]Package) map[string][]Package {
	var pkgs []Package
	if strings.HasPrefix(xmlname, "opensuse.12") {
		...
		pkgs = walkCriteria(cri, tests)
	}
	verPkgs := map[string][]Package{}
	for _, p := range pkgs {
		v, err := getOSVersion(p.Platform)
		if err != nil {
			log.Printf(`[WARN] failed to get os version. err: %s`, err)
			continue
		}
		if v == "" {
			continue
