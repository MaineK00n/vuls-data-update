...
func Fetch(opts ...Option) error {
	...
	for _, u := range options.urls {
		...
		for _, a := range feed.Vulinfo {
			if err := util.Write(filepath.Join(dir, fmt.Sprintf("%s.json", a.VulinfoID)), a); err != nil {
				return errors.Wrapf(err, "write %s", filepath.Join(dir, fmt.Sprintf("%s.json", a.VulinfoID)))
			}
