...
import (
	...
	"github.com/MaineK00n/vuls-data-update/pkg/fetch/other/nvd/feed"
)
...
func TestFetch(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			dir := t.TempDir()
			err = feed.Fetch(feed.WithFeedURL(&urls), feed.WithDir(dir), feed.WithRetry(0))
			switch {
