...
const dataURL = "https://www.sick.com/.well-known/csaf/white/csaf-feed-tlp-white.json"
...
func (opts options) fetchCSAF(client *utilhttp.Client, urls []string) error {
	if err := client.PipelineGet(urls, opts.concurrency, opts.wait, false, func(resp *http.Response) error {
		...
		t, err := time.Parse("2006-01-02T15:04:05.000Z", csaf.Document.Tracking.InitialReleaseDate)
		if err != nil {
			return errors.Wrapf(err, "unexpected published format. expected: %q, actual: %q", "2006-01-02T15:04:05.000Z", csaf.Document.Tracking.InitialReleaseDate)
		}

		p, err := utilfilepath.Join(opts.dir, fmt.Sprintf("%d", t.Year()), fmt.Sprintf("%s.json", csaf.Document.Tracking.ID))
		if err != nil {
