...
func Fetch(opts ...Option) error {
	...
	for _, ovalname := range ovals {
		...
		for _, test := range root.Tests.RpmverifyfileTest {
			p, err := utilfilepath.Join(options.dir, ver, "tests", "rpmverifyfile_test", fmt.Sprintf("%s.json", test.ID))
			if err != nil {
				return errors.Wrap(err, "join")
			}
		...
		for _, test := range root.Tests.UnameTest {
			p, err := utilfilepath.Join(options.dir, ver, "tests", "uname_test", fmt.Sprintf("%s.json", test.ID))
			if err != nil {
				return errors.Wrap(err, "join")
			}
		...
		for _, object := range root.Objects.RpmverifyfileObject {
			p, err := utilfilepath.Join(options.dir, ver, "objects", "rpmverifyfile_object", fmt.Sprintf("%s.json", object.ID))
			if err != nil {
				return errors.Wrap(err, "join")
			}
		...
		if root.Objects.UnameObject.ID != "" {
			p, err := utilfilepath.Join(options.dir, ver, "objects", "uname_object", fmt.Sprintf("%s.json", root.Objects.UnameObject.ID))
			if err != nil {
				return errors.Wrap(err, "join")
			}
		...
		for _, state := range root.States.RpmverifyfileState {
			p, err := utilfilepath.Join(options.dir, ver, "states", "rpmverifyfile_state", fmt.Sprintf("%s.json", state.ID))
			if err != nil {
				return errors.Wrap(err, "join")
			}
		...
		for _, state := range root.States.UnameState {
			p, err := utilfilepath.Join(options.dir, ver, "states", "uname_state", fmt.Sprintf("%s.json", state.ID))
			if err != nil {
				return errors.Wrap(err, "join")
			}
		...
		if root.Variables.LocalVariable.ID != "" {
			p, err := utilfilepath.Join(options.dir, ver, "variables", "local_variable", fmt.Sprintf("%s.json", root.Variables.LocalVariable.ID))
			if err != nil {
				return errors.Wrap(err, "join")
			}
