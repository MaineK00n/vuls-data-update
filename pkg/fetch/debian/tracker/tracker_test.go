...
import (
	...
	"github.com/MaineK00n/vuls-data-update/pkg/fetch/debian/tracker"
)
...
func TestFetch(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "unexpected codename",
			testdata: "testdata/fixtures/unexpected_code.json",
			hasError: true,
		},
		{
			name:     "404 not found",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			dir := t.TempDir()
			err := tracker.Fetch(tracker.WithAdvisoryURL(ts.URL), tracker.WithDir(dir), tracker.WithRetry(0))
			switch {
