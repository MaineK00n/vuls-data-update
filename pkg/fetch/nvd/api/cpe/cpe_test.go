...
func TestFetch(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			dir := t.TempDir()
			err = cpe.Fetch(cpe.WithBaseURL(u), cpe.WithDir(dir), cpe.WithRetry(0), cpe.WithConcurrency(3), cpe.WithWait(0), cpe.WithAPIKey(tt.apiKey), cpe.WithResultsPerPage(3))
			switch {
