...
func TestFetch(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			args: []cpe.Option{
				cpe.WithLastModStartDate(new(time.Date(2023, time.November, 14, 21, 35, 0, 0, time.UTC))),
				cpe.WithLastModEndDate(new(time.Date(2023, time.November, 14, 21, 40, 0, 0, time.UTC))),
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			dir := t.TempDir()
			err := cpe.Fetch(append(tt.args, cpe.WithHTTPClient(ts.Client()), cpe.WithDir(dir), cpe.WithRetry(0), cpe.WithConcurrency(3), cpe.WithWait(0), cpe.WithResultsPerPage(3))...)
			switch {
