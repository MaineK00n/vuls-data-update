...
func TestFetch(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			dir := t.TempDir()
			err = cpematch.Fetch(cpematch.WithBaseURL(u), cpematch.WithDir(dir), cpematch.WithRetry(0), cpematch.WithConcurrency(3), cpematch.WithWait(0), cpematch.WithAPIKey(tt.apiKey), cpematch.WithResultsPerPage(3))
			switch {
