...
func Fetch(opts ...Option) error {
	...
	options := &options{
		feedURL: &u,
		dir:     filepath.Join(util.SourceDir(), "nvd", "feed", "cve"),
		retry:   3,
	}
	...
	slices.SortFunc(options.feedURL.CVE, func(a, b string) int {
	...
	for _, u := range options.feedURL.CVE {
		...
		for _, cve := range cves[feedname] {
			...
			if err := util.Write(filepath.Join(options.dir, y, fmt.Sprintf("%s.json", cve.Cve.CVEDataMeta.ID)), cve); err != nil {
