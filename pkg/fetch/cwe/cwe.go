...
func Fetch(opts ...Option) error {
	...
	for _, w := range weaknesses {
		if err := util.Write(filepath.Join(dir, fmt.Sprintf("%s.json", w.ID)), w); err != nil {
			return errors.Wrapf(err, "write %s", filepath.Join(dir, fmt.Sprintf("%s.json", w.ID)))
		}
	...
	for _, c := range categories {
		if err := util.Write(filepath.Join(dir, fmt.Sprintf("%s.json", c.ID)), c); err != nil {
			return errors.Wrapf(err, "write %s", filepath.Join(dir, fmt.Sprintf("%s.json", c.ID)))
		}
	...
	for _, v := range views {
		if err := util.Write(filepath.Join(dir, fmt.Sprintf("%s.json", v.ID)), v); err != nil {
			return errors.Wrapf(err, "write %s", filepath.Join(dir, fmt.Sprintf("%s.json", v.ID)))
		}
