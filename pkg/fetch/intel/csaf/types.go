...
type CSAF struct {
	Comment         string          `json:"__comment__,omitempty"`
	Document        Document        `json:"document"`
	ProductTree     ProductTree     `json:"product_tree,omitzero"`
...
type Vulnerability struct {
	...
	Scores []struct {
		...
		// CSAF 2.0 does not define cvss_v4, but Intel embeds the CVSS v4.0 JSON schema here.
		// baseScore is null when Intel does not assign a CVSS v4.0 score, so it is kept as a pointer.
		CvssV4 *struct {
			AttackComplexity          string   `json:"attackComplexity,omitempty"`
