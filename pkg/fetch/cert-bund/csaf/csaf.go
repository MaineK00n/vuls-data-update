...
const baseURL = "https://wid.cert-bund.de/.well-known/csaf/white/"
...
func (o options) fetch() error {
	...
	if err := client.PipelineGet(us, o.concurrency, o.wait, false, func(resp *http.Response) error {
		...
		t, err := time.Parse(time.RFC3339Nano, csaf.Document.Tracking.InitialReleaseDate)
		if err != nil {
			return errors.Wrapf(err, "failed to parse InitialReleaseDate option. expected: %q, actual: %q", time.RFC3339Nano, csaf.Document.Tracking.InitialReleaseDate)
		}

		p, err := utilfilepath.Join(o.dir, fmt.Sprintf("%d", t.Year()), fmt.Sprintf("%s.json", csaf.Document.Tracking.ID))
		if err != nil {
