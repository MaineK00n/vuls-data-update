...
func (o options) fetchArchiveDate(client *utilhttp.Client) (time.Time, error) {
	...
	s := string(bs)
	if !strings.HasPrefix(s, "csaf_advisories_") || !strings.HasSuffix(s, ".tar.zst") {
		return time.Time{}, errors.Errorf("unexpected archive file name. expected: %q, actual: %q", "csaf_advisories_yyyy-mm-dd.tar.zst", s)
	}

	at, err := time.Parse("2006-01-02", strings.TrimPrefix(strings.TrimSuffix(s, ".tar.zst"), "csaf_advisories_"))
	if err != nil {
		return time.Time{}, errors.Wrapf(err, "time parse. unexpected archive time format. expected: %q. actual: %q", "2006-01-02", strings.TrimPrefix(strings.TrimSuffix(s, ".tar.zst"), "csaf_advisories_"))
	}
...
func (o options) fetchArchive(client *utilhttp.Client, archived time.Time) error {
	u, err := url.JoinPath(o.baseURL, fmt.Sprintf("csaf_advisories_%s.tar.zst", archived.Format("2006-01-02")))
	if err != nil {
	...
	for {
		...
		if err := util.Write(filepath.Join(o.dir, splitted[1], fmt.Sprintf("%s.json", csaf.Document.Tracking.ID)), csaf); err != nil {
			return errors.Wrapf(err, "write %s", filepath.Join(o.dir, splitted[1], fmt.Sprintf("%s.json", csaf.Document.Tracking.ID)))
		}
	}
...
func (o options) fetchDeletions(client *utilhttp.Client, archived time.Time) error {
	...
	for {
		...
		if rt.After(archived) {
			...
			if err := os.Remove(filepath.Join(o.dir, d, fmt.Sprintf("%s.json", strings.ToUpper(strings.ReplaceAll(strings.TrimSuffix(f, ".json"), "_", ":"))))); err != nil && !errors.Is(err, fs.ErrNotExist) {
				return errors.Wrapf(err, "remove %s", filepath.Join(o.dir, d, fmt.Sprintf("%s.json", strings.ToUpper(strings.ReplaceAll(strings.TrimSuffix(f, ".json"), "_", ":")))))
			}
		}
	}
