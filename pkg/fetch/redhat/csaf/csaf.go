...
func (o options) fetchArchive(client *utilhttp.Client, archived time.Time) error {
	...
	for {
		...
		p, err := utilfilepath.Join(o.dir, splitted[1], fmt.Sprintf("%s.json", csaf.Document.Tracking.ID))
		if err != nil {
...
func (o options) fetchDeletions(client *utilhttp.Client, archived time.Time) error {
	...
	for {
		...
		if rt.After(archived) {
			...
			p, err := utilfilepath.Join(o.dir, d, fmt.Sprintf("%s.json", strings.ToUpper(strings.ReplaceAll(strings.TrimSuffix(f, ".json"), "_", ":"))))
			if err != nil {
				return errors.Wrap(err, "join")
			}
