...
const dataURL = "https://osv-vulnerabilities.storage.googleapis.com/Android/all.zip"
...
func Fetch(opts ...Option) error {
	...
	for _, zf := range r.File {
		...
		lhs, _, _ := strings.Cut(a.ID, "-")

		if err := util.Write(filepath.Join(options.dir, lhs, fmt.Sprintf("%s.json", a.ID)), a); err != nil {
			return errors.Wrapf(err, "write %s", filepath.Join(options.dir, lhs, fmt.Sprintf("%s.json", a.ID)))
		}
	}
