...
const dataURL = "https://osv-vulnerabilities.storage.googleapis.com/GitHub%20Actions/all.zip"
...
func Fetch(opts ...Option) error {
	...
	for _, zf := range r.File {
		...
		if err := util.Write(filepath.Join(options.dir, fmt.Sprintf("%d", t.Year()), fmt.Sprintf("%s.json", a.ID)), a); err != nil {
			return errors.Wrapf(err, "write %s", filepath.Join(options.dir, fmt.Sprintf("%d", t.Year()), fmt.Sprintf("%s.json", a.ID)))
		}
	}
