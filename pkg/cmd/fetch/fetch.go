...
import (
	...
	cisaCOUN7ER "github.com/MaineK00n/vuls-data-update/pkg/fetch/cisa/coun7er"
	...
	mitreCVRF "github.com/MaineK00n/vuls-data-update/pkg/fetch/mitre/cve/cvrf"
	mitreV4 "github.com/MaineK00n/vuls-data-update/pkg/fetch/mitre/cve/v4"
	mitreV5 "github.com/MaineK00n/vuls-data-update/pkg/fetch/mitre/cve/v5"
	...
	nucleiAPI "github.com/MaineK00n/vuls-data-update/pkg/fetch/nuclei/api"
	...
	nvidiaCSAF "github.com/MaineK00n/vuls-data-update/pkg/fetch/nvidia/csaf"
...
func NewCmdFetch() *cobra.Command {
	...
	cmd.AddCommand(
		...
		newCmdCISACOUN7ER(), newCmdCISACSAF(), newCmdCISAKEV(),
...
func newCmdAnolisOVAL() *cobra.Command {
	options := &base{
	...
	cmd := &cobra.Command{
		Use:   "anolis-oval",
		Short: "Fetch Anolis OS OVAL data source",
		Example: heredoc.Doc(`
			$ vuls-data-update fetch anolis-oval
		`),
		Args: cobra.NoArgs,
		RunE: func(_ *cobra.Command, _ []string) error {
			if err := anolisOVAL.Fetch(anolisOVAL.WithDir(options.dir), anolisOVAL.WithRetry(options.retry)); err != nil {
				return errors.Wrap(err, "failed to fetch anolis oval")
			}
			return nil
		},
	}
...
func newCmdCertBundCSAF() *cobra.Command {
	options := &struct {
		base
		concurrency int
		wait        time.Duration
	...
	cmd := &cobra.Command{
		Use:   "cert-bund-csaf",
		Short: "Fetch Cert-Bund CSAF data source",
		Example: heredoc.Doc(`
			$ vuls-data-update fetch cert-bund-csaf
		`),
		Args: cobra.NoArgs,
		RunE: func(_ *cobra.Command, _ []string) error {
			if err := certBundCSAF.Fetch(certBundCSAF.WithDir(options.dir), certBundCSAF.WithRetry(options.retry), certBundCSAF.WithConcurrency(options.concurrency), certBundCSAF.WithWait(options.wait)); err != nil {
				return errors.Wrap(err, "failed to fetch cert-bund csaf")
			}
			return nil
		},
	}
...
func newCmdCISACOUN7ER() *cobra.Command {
	options := &base{
	...
	cmd := &cobra.Command{
		Use:   "cisa-coun7er",
		Short: "Fetch CISA COUN7ER data source",
		Example: heredoc.Doc(`
			$ vuls-data-update fetch cisa-coun7er
		`),
		Args: cobra.NoArgs,
		RunE: func(_ *cobra.Command, _ []string) error {
			if err := cisaCOUN7ER.Fetch(cisaCOUN7ER.WithDir(options.dir), cisaCOUN7ER.WithRetry(options.retry)); err != nil {
				return errors.Wrap(err, "failed to fetch cisa coun7er")
			}
			return nil
		},
	}
...
func newCmdK8sOSV() *cobra.Command {
	options := &base{
	...
	cmd := &cobra.Command{
		Use:   "k8s-osv",
		Short: "Fetch Kubernetes OSV data source",
		Example: heredoc.Doc(`
			$ vuls-data-update fetch k8s-osv
		`),
		Args: cobra.NoArgs,
		RunE: func(_ *cobra.Command, _ []string) error {
			if err := k8sOSV.Fetch(k8sOSV.WithDir(options.dir), k8sOSV.WithRetry(options.retry)); err != nil {
				return errors.Wrap(err, "failed to fetch k8s osv")
			}
			return nil
		},
	}
...
func newCmdMicrosoftWSUSSCN2() *cobra.Command {
	...
	cmd.Flags().StringVarP(&options.dir, "dir", "d", options.dir, "output fetch results to specified directory")
	cmd.Flags().IntVarP(&options.retry, "retry", "", options.retry, "number of retry http request")
	cmd.Flags().IntVarP(&options.concurrency, "concurrency", "", options.concurrency, "number of concurrent cabextract processes")
...
func newCmdMinimOSOSV() *cobra.Command {
	options := &base{
		dir:   filepath.Join(util.CacheDir(), "fetch", "minimos", "osv"),
		retry: 3,
	}

	cmd := &cobra.Command{
		Use:   "minimos-osv",
		Short: "Fetch MinimOS OSV data source",
		Example: heredoc.Doc(`
			$ vuls-data-update fetch minimos-osv
		`),
		Args: cobra.NoArgs,
		RunE: func(_ *cobra.Command, _ []string) error {
			if err := minimOSOSV.Fetch(minimOSOSV.WithDir(options.dir), minimOSOSV.WithRetry(options.retry)); err != nil {
				return errors.Wrap(err, "failed to fetch minimos osv")
			}
			return nil
		},
	}
...
func newCmdNCSCCSAF() *cobra.Command {
	options := &struct {
		base
		concurrency int
		wait        time.Duration
	...
	cmd := &cobra.Command{
		Use:   "ncsc-csaf",
		Short: "Fetch Netherlands Cyber Security Center CSAF data source",
		Example: heredoc.Doc(`
			$ vuls-data-update fetch ncsc-csaf
		`),
		Args: cobra.NoArgs,
		RunE: func(_ *cobra.Command, _ []string) error {
			if err := ncscCSAF.Fetch(ncscCSAF.WithDir(options.dir), ncscCSAF.WithRetry(options.retry), ncscCSAF.WithConcurrency(options.concurrency), ncscCSAF.WithWait(options.wait)); err != nil {
				return errors.Wrap(err, "failed to fetch ncsc csaf")
			}
			return nil
		},
	}
...
func newCmdNucleiAPI() *cobra.Command {
	options := &base{
	...
	cmd := &cobra.Command{
		Use:   "nuclei-api <api-key>",
		Short: "Fetch Nuclei Templates API data source",
		Example: heredoc.Doc(`
			$ vuls-data-update fetch nuclei-api key12345
		`),
		Args: cobra.ExactArgs(1),
		RunE: func(_ *cobra.Command, args []string) error {
			if err := nucleiAPI.Fetch(args[0], nucleiAPI.WithDir(options.dir), nucleiAPI.WithRetry(options.retry)); err != nil {
				return errors.Wrap(err, "failed to fetch nuclei api")
			}
			return nil
		},
	}
...
func newCmdNVIDIACSAF() *cobra.Command {
	options := &base{
	...
	cmd := &cobra.Command{
		Use:   "nvidia-csaf",
		Short: "Fetch NVIDIA CSAF data source",
		Example: heredoc.Doc(`
			$ vuls-data-update fetch nvidia-csaf
		`),
		Args: cobra.NoArgs,
		RunE: func(_ *cobra.Command, _ []string) error {
			if err := nvidiaCSAF.Fetch(nvidiaCSAF.WithDir(options.dir), nvidiaCSAF.WithRetry(options.retry)); err != nil {
				return errors.Wrap(err, "failed to fetch nvidia csaf")
			}
			return nil
		},
	}
...
func newCmdVulnCheckNISTNVD2() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "vulncheck-nist-nvd2 <VulnCheck token>",
		Short: "Fetch VulnCheck NVD++ (nist-nvd2) data source",
		Example: heredoc.Doc(`
			$ vuls-data-update fetch vulncheck-nist-nvd2 vulncheck_token
		`),
		Args: cobra.ExactArgs(1),
		RunE: func(_ *cobra.Command, args []string) error {
			if err := vulncheckNISTNVD2.Fetch(args[0], vulncheckNISTNVD2.WithDir(options.dir), vulncheckNISTNVD2.WithRetry(options.retry)); err != nil {
				return errors.Wrap(err, "failed to fetch vulncheck nist-nvd2")
			}
			return nil
		},
	}
