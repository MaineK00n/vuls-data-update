...
func newCmdRedHatPackageManifest() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Args: cobra.NoArgs,
		RunE: func(_ *cobra.Command, _ []string) error {
			if err := redhatPackageManifest.Fetch(
				redhatPackageManifest.WithDir(options.dir),
				redhatPackageManifest.WithRetry(options.retry),
				redhatPackageManifest.WithMajors(options.majors...),
			); err != nil {
	...
	cmd.Flags().StringVarP(&options.dir, "dir", "d",
		filepath.Join(util.CacheDir(), "fetch", "redhat", "packagemanifest"),
		"output fetch results to specified directory")
	cmd.Flags().IntVarP(&options.retry, "retry", "", 3, "number of retry http request")
	cmd.Flags().IntSliceVar(&options.majors, "majors", []int{8, 9, 10}, "target RHEL major versions (comma-separated)")
