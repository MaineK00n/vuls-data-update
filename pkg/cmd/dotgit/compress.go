...
func newCmdCompress() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		PreRunE: func(_ *cobra.Command, args []string) error {
			if options.output == "" {
				options.output = fmt.Sprintf("%s.tar.zst", filepath.Base(args[0]))
			}

			if options.root == "" {
				options.root = filepath.Base(args[0])
			}

			return nil
		},
		RunE: func(_ *cobra.Command, args []string) error {
			if err := compress.Compress(args[0], options.output, compress.WithRoot(options.root)); err != nil {
