...
func Extract(vexDir, repository2cpeDir string, opts ...Option) error {
	...
	if err := filepath.WalkDir(vexDir, func(path string, d fs.DirEntry, err error) error {
		...
		p, err := utilfilepath.Join(options.dir, "data", ss[1], fmt.Sprintf("%s.json", extracted.ID))
		if err != nil {
				...
				if fpn.ProductIdentificationHelper.PURL != "" {
					...
					default:
						...
						for _, s := range []string{
							"pkg:cargo/", "pkg:gem/", "pkg:generic/", "pkg:github/", "pkg:golang/", "pkg:luarocks/", "pkg:maven/", "pkg:npm/", "pkg:oci/", "pkg:pypi/",
						} {
