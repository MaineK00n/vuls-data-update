...
func TestCondition_Filter(t *testing.T) {
	type fields struct {
		Criteria criteriaTypes.Criteria
		Tag      segmentTypes.DetectionTag
	}
	...
	tests := []struct {
		...
		},
		{
			...
			args: args{
				query: criterionTypes.Query{
					...
					NoneExist: &necTypes.Query{
						Binaries: []necBinaryPackageTypes.Query{{Name: "package1"}, {Name: "package2"}},
						Sources:  []necSourcePackageTypes.Query{{Name: "package1"}, {Name: "package2"}},
					},
				},
			},
			want: nil,
		...
		{
			name: "OR - AND - {OR1, OR2}, OR1 is not satisfied (SUSE inspired)",
			fields: fields{
				Criteria: criteriaTypes.Criteria{
					Operator: criteriaTypes.CriteriaOperatorTypeOR,
					Criterias: []criteriaTypes.Criteria{
									...
									Criterions: []criterionTypes.Criterion{
										{
											...
											Version: &vcTypes.Criterion{
												...
												Affected: &affectedTypes.Affected{
													Type:  affectedrangeTypes.RangeTypeRPM,
													Range: []affectedrangeTypes.Range{{LessThan: "0.0.2-1"}},
												},
											},
										},
									},
			...
			args: args{
				query: criterionTypes.Query{
					Version: []vcTypes.Query{
						{
							Binary: &vcTypes.QueryBinary{
								Family:  ecosystemTypes.EcosystemTypeSUSEEnterpriseServer,
								Name:    "product2-release",
								Version: "0.0.2",
							},
						},
					...
					NoneExist: &necTypes.Query{
						Binaries: []necBinaryPackageTypes.Query{{Name: "product2-release"}, {Name: "package1"}, {Name: "package2"}},
						Sources:  []necSourcePackageTypes.Query{{Name: "product2-release"}, {Name: "package1"}, {Name: "package2"}},
					},
				},
			},
			want: &conditionTypes.FilteredCondition{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := c.Filter(tt.args.query)
			if (err != nil) != tt.wantErr {
