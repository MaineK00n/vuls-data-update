...
type Group struct {
	...
	CampaignsAttributed []string                           `json:"campaigns_attributed,omitempty"` // Campaign IDs ("C*") attributing to this group (reverse of campaign attributed-to)
}

func (g *Group) Sort() {
	slices.Sort(g.Aliases)
	slices.Sort(g.SoftwaresUsed)
	slices.Sort(g.CampaignsAttributed)
...
func Compare(x, y Group) int {
	return cmp.Or(
		...
		slices.Compare(x.CampaignsAttributed, y.CampaignsAttributed),
	)
}
