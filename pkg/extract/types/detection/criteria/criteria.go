...
func Compare(x, y Criteria) int {
	if c := cmp.Compare(x.Operator, y.Operator); c != 0 {
		return c
	}
	if c := slices.CompareFunc(x.Criterias, y.Criterias, Compare); c != 0 {
		return c
	}
	return slices.CompareFunc(x.Criterions, y.Criterions, criterion.Compare)
}
