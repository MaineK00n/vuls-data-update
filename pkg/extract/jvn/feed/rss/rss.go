...
func Extract(args string, opts ...Option) error {
	...
	if err := filepath.WalkDir(args, func(path string, d fs.DirEntry, err error) error {
		...
		p, err := utilfilepath.Join(options.dir, "data", splitted[1], fmt.Sprintf("%s.json", data.ID))
		if err != nil {
	...
	for id, c := range cwes {
		p, err := utilfilepath.Join(options.dir, "cwe", fmt.Sprintf("%s.json", id))
		if err != nil {
			return errors.Wrap(err, "join")
		}

		if err := util.Write(p, c, true); err != nil {
			return errors.Wrapf(err, "write %s", p)
		}
	}
