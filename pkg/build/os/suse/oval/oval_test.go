...
import (
	...
	"github.com/MaineK00n/vuls-data-update/pkg/build/os/suse/oval"
)
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := filepath.Walk(d, func(path string, info fs.FileInfo, err error) error {
				...
				if filepath.Base(dir) == "vulnerability" {
					...
					wantb, err := os.ReadFile(filepath.Join("testdata", "golden", "os", "suse", "oval", filepath.Base(dir), v, y, file))
					if err != nil {
						return err
					}
