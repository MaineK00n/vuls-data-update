...
func Build(opts ...Option) error {
	options := &options{
		srcDir:             filepath.Join(util.SourceDir(), "nuget", "osv"),
		srcCompressFormat:  "",
		destVulnDir:        filepath.Join(util.DestDir(), "vulnerability"),
		destDetectDir:      filepath.Join(util.DestDir(), "library", "nuget", "osv"),
		destCompressFormat: "",
	}
	...
	log.Println("[INFO] Build Nuget OSV")
	...
	if err := filepath.WalkDir(options.srcDir, func(path string, d fs.DirEntry, err error) error {
		...
		return errors.Wrap(err, "walk osv")
	}
